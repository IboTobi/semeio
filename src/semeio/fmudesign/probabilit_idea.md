...
In this simplistic example with discrete probabilities, the objective function will be the sum of some payoff values generated from the decision to acquire seismic [Yes/No], the decision to drill [Yes/No], and the uncertainty about the reserves [Dry/Wet/Soaking]. It is straightforward to calculate these payoffs based on given values, but if this example is extended a Monte Carlo simulation will be needed. 

An extension of this example is to use a continuous probability distribution for the reserves uncertainty, add an uncertain oil price, and an uncertainty in drilling cost. Going one step further, the seismic might tell us not directly anything about the reserves amount but rather of other properties which are uncertain. Given this, we can use Monte Carlo simulation to estimate a distribution for the STOIIP which can then be used as a distribution for the reserves. Evaluating this can give a better picture on whether to acquire seismic and if the reservoir should be drilled or not based on the results from the seismic. This approach can also be transformed into an exercise to estimate the value of information of the seismic acquisition.
